...
function initializeSpaceCamp() {
	...
	else if (window.location.hostname.toLowerCase() === "3.basecamp.com") {
...
/**
 * Calculates the total hours in a Basecamp list. Also shows how much of the items are finished already.
 */
...
function renderBasecampTimeButton() {
	...
	var render = function () {
		...
		url = url.replace("{title}", document.title.replaceAll("#", "%23").replaceAll(",", "%2C").replaceAll(" ", "%20").replaceAll("&", "and"));
...
/**
 * Puts the description from the url in the description textarea of TimeChimp
 */
