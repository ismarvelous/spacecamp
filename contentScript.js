...
function initializeSpaceCamp() {
    ...
    if (hostname === "app.timechimp.com") {
        addDescription();
    } else if (hostname === "3.basecamp.com") {
...
function renderBasecampTimeButton() {
    ...
    const render = () => {
        ...
        let url = timeUrlPattern.replace("{labels}", encodeURIComponent(jLabels))
                                .replace("{title}", encodeURIComponent(document.title.replace("&", "and")));
        ...
        const menuitm = `
            <li id="spcamp-time" class="nav__item" data-menu-section="time">
                <a class="nav__link" href="${url}" target="time">
...
function addDescription() {
    ...
    if (!urlParams.has("text")) return;

    const textarea = document.querySelector("textarea[ng-model='vm.time.notes']");
    if (textarea) {
